...
| DEV-050 | `DeleteAsync` | The instance `DeleteTab` (L446–457) fetched children via `GetTabsByParentId(TabId, PortalId)` and only deleted when **none** remained (`arrTabs.Count = 0`); when children existed it **silently fell through** (no deletion, no error) | The same guard is enforced at the service layer, but a parent tab with children surfaces an explicit `InvalidOperationException("Cannot delete a tab that has child tabs.")` → RFC 7807 via middleware | Behavior change (surfaced error vs silent skip): the silent skip would mislead a caller into believing the parent was removed; mirrors the `PortalService` last-portal guard convention. The guard lives in the service because `ITabRepository.DeleteAsync` is intentionally unconditional |
