...
- **Three non-physical properties carried as scalars.** `HasChildren` / `AuthorizedRoles` / `AdministratorRoles` were computed / permission-derived at runtime in legacy (they appear only as stored-procedure `@parameters`, never as `Tabs` columns). They are settable auto-properties, so they are carried as ordinary mapped scalars (**not** `Ignore`d) for Phase-1 round-trip fidelity — no schema change (ADR-002), mirroring the `PortalConfiguration` / `PermissionConfiguration` precedent.
...
- **Denormalized `Module` carried as scalars.** Legacy `ModuleInfo` is a fat object hydrated from a JOIN across `Modules` + `TabModules` + `ModuleControls` + `DesktopModules` + `ModuleDefinitions`. Only **eleven** properties are real `Modules` columns (`ModuleID`, `ModuleDefID`, `ModuleTitle`, `AllTabs`, `IsDeleted`, `InheritViewPermissions`, `Header`, `Footer`, `StartDate`, `EndDate`, `PortalID`). The remaining join-sourced properties have no physical column on `Modules` and are carried as scalar properties (**not** `Ignore`d) for Phase-1 round-trip fidelity: the `TabModules` group (`TabModuleID`, `TabID`, `PaneName`, `ModuleOrder`, `CacheTime`, `Alignment`, `Color`, `Border`, `IconFile`, `Visibility`, `ContainerSrc`, `DisplayTitle`, `DisplayPrint`, `DisplaySyndicate`); the `ModuleControls` group (`ModuleControlId`, `ControlSrc`, `ControlType`, `ControlTitle`, `HelpUrl`, `SupportsPartialRendering`); and the `DesktopModules`/`ModuleDefinitions` group (`DesktopModuleID`, `FriendlyName`, `FolderName`, `Description`, `Version`, `IsPremium`, `IsAdmin`, `BusinessControllerClass`, `ModuleName`, `SupportedFeatures`). Mapping convenience only - no schema change (ADR-002). This mirrors the section 4.6 `User` flatten and the section 6.4 `Permission` Deviation-4 carry precedent.
...
- **`Module.Visibility` enum -> int.** `Visibility` is the `VisibilityState` enum (`Maximized=0`, `Minimized=1`, `None=2`, see section 4.5); EF maps enums to their underlying `int` by convention. `HasConversion<int>()` is declared explicitly for clarity and is in-memory-provider-safe.
...
- **`DesktopModule` carried members.** Eleven real `DesktopModules` columns are mapped (`DesktopModuleID`, `FriendlyName`, `Description`, `Version`, `IsPremium`, `IsAdmin`, `BusinessControllerClass`, `FolderName`, `ModuleName`, `SupportedFeatures`, `CompatibleVersions`). `IsUpgradeable`/`IsPortable`/`IsSearchable` (legacy-derived at runtime from the `SupportedFeatures` bitmask) and `Dependencies`/`Permissions` (absent from the 4.9 baseline schema) are carried as scalars (**not** `Ignore`d) for Phase-1 fidelity.
- **`ModuleDefinition` carried member.** Four real `ModuleDefinitions` columns are mapped (`ModuleDefID`, `FriendlyName`, `DesktopModuleID`, `DefaultCacheTime`). `DesktopModuleID` is a plain scalar FK - no `ModuleDefinition -> DesktopModule` navigation exists, so no relationship is configured. The runtime-only `TempModuleID` is carried as a scalar (**not** `Ignore`d).
...
| DEV-029 | `RefreshAsync` | Forms Authentication had **no** refresh-token concept | Refresh-token validation and rotation are delegated entirely to `IJwtService`: validate the presented token → resolve the subject from `ClaimTypes.NameIdentifier` (falling back to the `sub` claim) → reload the user via `IUserRepository.GetByIdAsync` → issue a **fresh access + refresh pair** | Stateless rotation aligned with [§3.1](#31-session-management-forms-authentication--stateless-jwt-bearer); a persistent server-side refresh-token store is out of Phase-1 scope, so validity is established cryptographically by `IJwtService` |
