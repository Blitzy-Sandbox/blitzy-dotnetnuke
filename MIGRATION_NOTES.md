...
| D-041 | User-profile membership actions (`frontend/.../features/user/components/user-profile/`) | `Membership.ascx.vb` exposed four command buttons — Authorize, Unauthorize, Unlock (cmdUnLock → `UserController.UnLockUser`), and Force Password Change (cmdPassword → `UpdateUser`). An earlier CP5 revision reproduced all four, but only `approved` (Authorize/Unauthorize) has a persistable contract, so Unlock and Force Password Change mutated **local-only** optimistic state that was lost on reload. | The **Unlock** and **Force Password Change** buttons and their dead-only logic are **removed** — `canUnlock`/`canForcePassword` computeds, the `unlock`/`force-password` members of the `MembershipAction` union, their `actionConfigs` entries, and their `changeFor` cases. Authorize/Unauthorize (persisted via `UpdateUserRequest.approved` through `UserService.updateUser`) are retained, as is the read-only membership STATUS display (approved/lockedOut/updatePassword). The spec drops the four removed-action tests. | Per **D-034**, the modern `UsersController` exposes list/get/create/update/delete only — there is no `/unlock` or `/force-password-change` endpoint and `UpdateUserRequest` carries no `lockedOut`/`updatePassword` field. Rather than fabricate non-persisting actions or out-of-Phase-1 backend endpoints, the two unsupported buttons are removed pending a persisted membership contract (CP5 user-profile parity finding). Behavior-honest; no backend change | N |
