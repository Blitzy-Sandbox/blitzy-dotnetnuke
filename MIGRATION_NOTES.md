...
**Faithfully preserved (behavioral equivalence).** The list reads (`GetByPortalAsync`, `GetByParentAsync`) return only non-deleted tabs; the legacy data layer filtered `IsDeleted` server-side, and that responsibility now lives in `ITabRepository` (`IsDeleted == false`). `TabService` never re-introduces deleted rows. `GetCountAsync` preserves the legacy `GetTabCount` scalar semantics.
