...
- **UI functional parity is required.** Every in-scope legacy administrative workflow
  ...
  (AAP §0.3.1) and cannot be persisted without a schema change that ADR-002 / AAP §0.2.2
  forbid. That is a documented scope reduction (see [D-034](#62-deviation-index) /
  [D-041](#62-deviation-index)); the **Force Password Change** transition, which maps to
  the real `dbo.Users.UpdatePassword` column, **is** reproduced end-to-end.
...
- **Refresh-token format (CP2 auth-chain fix).** The refresh token is a **signed JWT**
  ...
  the CP2 code-review finding. **CP-FINAL update (Code-Review G2):** rotation is now
  **stateful and revoking** — a server-side `IRefreshTokenStore` tracks each refresh
  token by **family** plus per-token `jti`, so `AuthService.RefreshAsync` calls
  `TryRotate(family, presentedJti)` to invalidate the consumed token id and mint a
  successor within the same family; a presented **superseded** id is treated as replay
  and revokes the entire family. Consequently `LogoutAsync(userId)` **revokes every
  refresh-token family** for the user (it is **no longer a no-op**), and `AuthController`
  deletes the `dnn_refresh_token` cookie. The store is in-memory (single-instance
  deployment); a distributed/persistent cache is the documented horizontal-scale upgrade
  (see [D-028](#62-deviation-index) / [D-029](#62-deviation-index)).
...
batched/streamed processing is **intentionally not done**: it would alter the legacy
all-users / all-members semantics this migration must reproduce verbatim (AAP §0.7.1,
*"Logic is ported as-is"*; the Minimal Change Clause forbids optimizing ported behavior),
and it would require widening the `IRoleRepository` / `IUserRepository` contracts and
adjusting `RoleService`. Any future batching is therefore a **post-migration performance
enhancement**, not deferred migration scope. The user
...
| D-027 | Auth login (`AuthService.LoginAsync`) | `UserController.UserLogin(portalId, …)` was portal-scoped, ran `ValidateUser`, then `FormsAuthentication.SetAuthCookie` (`UserController.vb` L991–L1033) | `LoginRequestDto` carries no portal context → defaults to the DNN primary portal (`PortalID = 0`); password verified via BCrypt `IPasswordHasher.Verify`; a JWT access token is returned in the response body while a **new refresh-token family** is registered in `IRefreshTokenStore` and the refresh token is delivered **only via an HttpOnly + Secure + SameSite=Strict cookie** (`dnn_refresh_token`, path `/api/auth`) — never in the JSON body (CP-FINAL / G3); a generic `UnauthorizedAccessException` avoids user enumeration | Facet of the sanctioned auth change (see D-001/D-002); default-portal login is a Phase-1 simplification (no portal selector in scope); secure non-JS-readable refresh storage per AAP §0.7.2 | **Y** |
| D-028 | Auth logout (`AuthService.LogoutAsync`) | `PortalSecurity.SignOut()` called `FormsAuthentication.SignOut()` and expired the auth/role/language cookies (`PortalSecurity.vb` L77–L95) | **CP-FINAL update (G2):** `LogoutAsync(userId)` now **revokes every refresh-token family** for the user in `IRefreshTokenStore` (a stolen refresh token can no longer be rotated after logout), and `AuthController` **deletes the `dnn_refresh_token` cookie**; the short-lived stateless access token is discarded by the client. The method stays non-`async` (the in-memory store revoke is synchronous), keeping the Gate-1 build free of CS1998 | Facet of the sanctioned auth change (see D-001); the earlier no-op was replaced because a no-revocation logout left refresh tokens replayable (G2) | **Y** |
| D-029 | Auth refresh (`AuthService.RefreshAsync`) | No legacy equivalent — Forms Auth used persistent cookies/tickets, not refresh tokens (`UserController.vb` L1035–L1045) | **CP-FINAL update (G2):** the refresh token is read from the `dnn_refresh_token` HttpOnly cookie (JSON-body fallback retained for non-browser clients), validated by `IJwtService.ValidateToken`, then **rotated against a server-side `IRefreshTokenStore`** keyed by token-**family** + per-token `jti`: `TryRotate(family, presentedJti)` revokes the consumed id and registers the freshly minted one, and **replay of a consumed/revoked token is detected and revokes the entire family**. Subject resolved from `ClaimTypes.NameIdentifier` with a `"sub"` fallback; a fresh access+refresh pair is issued and the new refresh cookie is set | New capability under the sanctioned JWT model (see D-001); replaces the earlier non-revoking stateless rotation, which left a rotated token valid until expiry (G2). The Phase-1 store is in-memory (single-instance); a distributed cache is the documented horizontal-scale upgrade | **Y** |
...
| D-033 | Tab count (`TabRepository.GetCountAsync`) | `GetTabCount` ran `SELECT COUNT(*) - 1 FROM Tabs WHERE PortalID = @p AND TabID <> @AdminTabId AND (ParentId <> @AdminTabId OR ParentId IS NULL)`, with `@AdminTabId` loaded from `Portals.AdminTabId` (`SqlDataProvider` `GetTabCount` proc; `TabController.vb` L512-514) | **CP3 correction:** `GetCountAsync` now reproduces the proc verbatim — it loads the portal's `AdminTabId`, counts the portal's tabs excluding the admin tab itself (`TabID <> @AdminTabId`) and the admin tab's **direct children** (`ParentId <> @AdminTabId OR ParentId IS NULL`), and subtracts the legacy `- 1` offset. The legacy proc applies **no `IsDeleted` filter**, so the count deliberately includes soft-deleted tabs (unlike the `GetByPortalAsync`/`GetByParentAsync` list reads, which filter `!IsDeleted`). The `@AdminTabId IS NULL` case (nullable column / missing portal) reproduces T-SQL three-valued logic — `TabID <> NULL` is UNKNOWN for every row, so the count is 0 and the method returns `-1` — handled with an explicit branch rather than C#/EF null-comparison semantics (which would treat `TabID != null` as TRUE for all rows under the InMemory provider). | An earlier revision returned a straight `COUNT(*)` of non-deleted tabs with **no admin-tab exclusion and no `- 1` offset** (the CP3 Tab-count behavioral-equivalence finding), and wrongly placed the exclusion in the service layer instead of the repository. The repository owns the parity because it is the layer that can load `Portals.AdminTabId` (`TabService` now exists and delegates the count to `TabRepository.GetCountAsync`). Behavior-preserving per the Minimal Change Clause; InMemory-safe (Gate 5) | N |
| D-034 | User feature service contract (`frontend/.../features/user/services/user.service.ts`, `models/user.model.ts`) | The frontend `UserService` exposed four membership-transition methods — `approveUser` / `unauthorizeUser` / `unlockUser` / `forcePasswordChange` — calling `PUT /api/v1/users/{id}/{approve,unauthorize,unlock,force-password-change}`, and it sent the feature **form** models `CreateUserDto` / `UpdateUserDto` as the request bodies. Those form models carry form-only fields (`authorize`, `confirmPassword`, `randomPassword`, `question`, `answer`, `notify`, `affiliateId`) and omit `portalID` / `isSuperUser` / `approved` / `userID`. They derive from the legacy `Membership.ascx.vb` command buttons and the `User.ascx.vb` create/edit flows. | **CP-FINAL update (G5) — supersedes the CP3 deferral.** Of the four legacy membership transitions, every one with a **persistable** contract is now implemented end-to-end: **Approve/Unauthorize** via `UpdateUserRequest.approved` (→ `dbo.Users` projection), and **Force Password Change** via a dedicated `POST /api/v1/users/{id}/force-password-change` (body `{ require: bool }`) backed by `IUserService.SetForcePasswordChangeAsync` → the real `dbo.Users.UpdatePassword` column, surfaced on the client by `user.service.ts#forcePasswordChange(id, require)`. The earlier "scoped to a later checkpoint" deferral is therefore **overturned** (this IS the FINAL checkpoint). **Account Unlock** — and durable persistence of the `Approved`/`LockedOut` flags themselves — remains a **permanent, AAP-grounded scope reduction**: those fields live on the GUID-keyed `aspnet_Membership` store, which is not a modeled Phase-1 entity (AAP §0.3.1) and cannot be persisted without a schema change forbidden by ADR-002 / AAP §0.2.2. `/unlock` therefore does not exist by design (documented here rather than silently dropped); `/approve`, `/unauthorize`, and `/force-password-change` semantics are delivered through `update` + the force-password endpoint. (2) Two **wire** request types were introduced — `CreateUserRequest` and `UpdateUserRequest` — that mirror the backend `CreateUserDto` / `UpdateUserDto` 1:1 under System.Text.Json camelCase (`portalID`, `username`, `password?`, `displayName`, `email`, `firstName`, `lastName`, `isSuperUser`, `approved` for create; `userID`, `displayName`, `email`, `firstName`, `lastName`, `isSuperUser`, `approved` for update). `UserService.createUser` / `updateUser` now accept the wire types, and `updateUser` stamps the route id onto the body's `userID` so `UsersController.Update` (which returns `400 "Identifier mismatch"` when `id != request.UserID`) always accepts the request. The form models are retained as the reactive-form binding shapes, and the user-form component adapts them into the wire types at the component boundary via its `toCreateRequest()` / `toUpdateRequest()` methods (implemented now: the create path calls `toCreateRequest` before `UserService.createUser`, and the update path calls `toUpdateRequest`, stamping the route id onto `userID`, before `UserService.updateUser`). | The client surface must match the actual controller routes and DTO contracts; the backend CRUD contracts are the authoritative CP1/CP2 deliverable and are preserved unchanged; the one G5 backend addition is the dedicated `force-password-change` endpoint (see this row's change column). Behavior-preserving for the implemented CRUD + force-password surface; the single unsupported transition (account **Unlock**) is documented as an AAP-grounded scope reduction rather than silently dropped | N |
...
| D-040 | Role-assignment UI date window (`frontend/.../features/role/components/role-assignment/`) | The admin `SecurityRoles.ascx.vb` (L528-542) collected an **effective** and an **expiry** date for each user→role assignment (a blank date textbox → `Null.NullDate` → null). An earlier CP5 revision of `role-assignment.component.ts` carried comments asserting "the REST API takes no body and there is no user-role DTO," and consequently OMITTED the date controls and sent no window. | The component now renders **Effective date** + **Expiry date** inputs bound to `effectiveDate`/`expiryDate` signals (native `[value]` + `(change)`, `'' → null`); `onAddUser()` passes `{ effectiveDate, expiryDate }` to `RoleService.assignUserToRole(roleId, userId, …)` and resets both on success. The factually-wrong "no body / no DTO" comments and class JSDoc are corrected. | The prior comments contradicted the contract already established in **D-026** — `role.service.ts#assignUserToRole` builds and POSTs `AssignUserRole { userID, roleID, effectiveDate, expiryDate }`, and the backend `AssignUserRoleDto`/`UserRoleAssignmentDto` accept it. This closes the last hop (the UI) so the admin assignment window persists, restoring legacy parity (CP5 role-assignment finding). Behavior-preserving alignment to D-026; no backend change | N |
| D-041 | User-profile membership actions (`frontend/.../features/user/components/user-profile/`) | `Membership.ascx.vb` exposed four command buttons — Authorize, Unauthorize, Unlock (cmdUnLock → `UserController.UnLockUser`), and Force Password Change (cmdPassword → `UpdateUser`). An earlier CP5 revision reproduced all four, but only `approved` (Authorize/Unauthorize) has a persistable contract, so Unlock and Force Password Change mutated **local-only** optimistic state that was lost on reload. | **CP-FINAL update (G5) — supersedes the CP5 removal.** The **Force Password Change** workflow is **restored** with a real persisted contract: the user-profile now renders a mutually-exclusive **Force Password Change** / **Clear Password-Change Requirement** button pair (gated on the `membership.updatePassword` status and hidden on one's own account) that calls `UserService.forcePasswordChange(id, require)` → `POST /api/v1/users/{id}/force-password-change`, optimistically toggling `membership.updatePassword`. Authorize/Unauthorize (persisted via `UpdateUserRequest.approved` through `UserService.updateUser`) are retained, as is the read-only membership STATUS display (approved/lockedOut/updatePassword). Only the **Unlock** button stays removed. The spec re-adds force/clear tests (own-account hidden, optimistic toggle). | Force Password Change now has a persisted endpoint (D-034 → `dbo.Users.UpdatePassword`), so the FINAL checkpoint **restores** it (the earlier "removed pending a persisted membership contract" rationale is satisfied and overturned). **Unlock** has no persistable contract (`aspnet_Membership`, GUID-keyed, ADR-002 / AAP §0.2.2) and stays removed as a documented scope reduction. Behavior-honest; the persisted half is delivered, no fabricated endpoint | N |
...
**Frontend list/assignment components (CP5) notes.** Rows D-040…D-044 capture the CP5
Angular list/assignment slice. The five standalone list/assignment components consume the shared
`data-table` conduit, which now uses **real CDK virtualization** (`*cdkVirtualFor`, D-042) per AAP
§0.3.4. Two components were brought into line with already-correct backend contracts: `role-assignment`
now sends the admin **effective/expiry window** to `RoleService.assignUserToRole` (D-040, aligning the UI
to the D-026 service/DTO/controller contract), and `user-list` now derives and sends the required
`portalId` from `AuthService.currentUser()?.portalID` (D-044, the `UsersController.Get` 400 guard). Where a
backend change would have risked the passed CP1-CP4 surface, the UI adapts instead: `module-list` performs
**client-side** filter/search/sort/paging over a single `getModulesByPortal(portalId)` read because
`ModulesController.Get` accepts only `portalId`/`tabId` (D-043). Finally, `user-profile` **removes** the
...
- **Why an exception and not an upgrade:** every advisory's `fixAvailable` resolves to a
  **breaking major** — `@angular/core` `21.2.17` (`isSemVerMajor: true`); the first patched
  ...
  patch** — the vulnerable range is `<= 19.2.25`, which is the entire 19 line as published.
  Upgrading off 19 is **forbidden by the AAP**: §0.5.1 pins `@angular/* ^19.0.0` and §0.7.2
  states *"Mandated versions are honored exactly regardless of newer releases."* A jump to
  Angular 20/21 is a multi-major migration well outside this rewrite's frozen scope. Per the
  §7.1 precedence note, the secure-dependency gate would normally force an upgrade — **but
  only when a compatible patched version exists** (as it did for AutoMapper in item (b),
  fixed in-place at 15.1.1). Here the only fix is out-of-scope and breaking, so the
  resolution is an **explicit, AAP-grounded accepted exception** carried by per-advisory
  compensating controls plus a defense-in-depth CSP (below).
- **Per-advisory assessment** against this application's actual code surface:
...
edge enforces a strict Content-Security-Policy in `docker/nginx.conf` (`default-src 'self';
script-src 'self'; object-src 'none'; frame-ancestors 'none'; base-uri 'self';
form-action 'self'`). `script-src 'self'` blocks injected inline/remote script and
`object-src 'none'` blocks plugin vectors, which materially reduces the residual impact of
any client-side XSS/DOM-clobbering advisory even where the analysis already shows the
vulnerable path is unused.

**Upgrade path (forward control).** When the AAP scope is next revised to permit a major
Angular bump, upgrade the whole `@angular/*` line to the patched **20.3.x (or 21.2.x)**
baseline in one coordinated step (CLI + devkit + compiler-cli together), then re-run
`npm audit --omit=dev` to confirm **0 production vulnerabilities** and `ng build`/`ng test`
to confirm no regression. Until then this exception stands and is re-validated each
checkpoint by re-capturing the `npm audit --omit=dev` totals above.
...
- **Container secret hardening (CP5).** `docker/docker-compose.yml` now **requires**
  ...
  password and a deterministic JWT signing key into a `Production` image), so
  `docker compose build`/`up` aborts when either is unset. Sample **non-secret** values
  move to `docker/.env.example`, and `docker/.gitignore` ignores `.env` / `.env.*` while
  keeping `.env.example` tracked (CWE-798). Companion CP5 container hardening: the frontend
