...
// maxlength limits) and ManageTabs.ascx.vb. Behavioral parity (AAP 0.7.1). TabInfo.vb carries only XmlElement
// attributes, so the rules originate from the markup. Synchronous-only validator (no repository/data access):
// duplicate tab-name detection (TabController.GetTabByName) is enforced in TabService via ITabRepository.GetByNameAsync.
...
public sealed class CreateTabValidator : AbstractValidator<CreateTabRequest>
{
