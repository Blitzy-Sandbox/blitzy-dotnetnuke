...
public interface IAuthService
{
    ...
    /// <summary>Logs the current authenticated user out (revokes their refresh token(s)).</summary>
    // MIGRATION: legacy PortalSecurity.SignOut(); with JWTs there is no server session to drop, so the
    // implementation revokes the user's stored refresh token(s), identified from the ClaimsPrincipal.
