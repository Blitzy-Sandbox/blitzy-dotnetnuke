...
// MIGRATION: Parity tests for UpdateTabValidator. Rules and messages are identical to
// CreateTabValidator (UpdateTabRequest simply omits PortalId), migrated from legacy DNN page/tab
// validation (Website/admin/Tabs/ManageTabs.ascx.vb), TabInfo.vb, and TabController.vb.
// NOTE: "Tab Name Is Required" has NO trailing period. Tests encode parity ONLY.
public sealed class UpdateTabValidatorTests
{
    private readonly UpdateTabValidator _validator = new();

    private const string TabNameRequired = "Tab Name Is Required";

    private static UpdateTabRequest Valid() => new()
    {
        TabName = "Home"
    };
