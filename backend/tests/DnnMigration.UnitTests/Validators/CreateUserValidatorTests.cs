...
public class CreateUserValidatorTests
{
    ...
    // Fully-valid baseline DTO so each test mutates exactly ONE field. The remaining CreateUserDto
    // members (PortalID, IsSuperUser, Approved) stay at their defaults and are not exercised by
    // CreateUserValidator, so the baseline yields zero validation errors.
    ...
    // FluentValidation 11.x EmailAddress() (AspNetCoreCompatible) fails values with no '@';
    // do NOT use "a@b"-style values here — those would pass.
    [Theory]
    ...
    // while the address remains a valid single-'@' format, so the error is attributable to length.
    [Fact]
