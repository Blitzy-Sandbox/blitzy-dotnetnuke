...
// MIGRATION: Verifies UpdatePortalValidator parity (PortalController.vb UpdatePortalInfo path). PortalID must be a
// valid existing key (>0); PortalName required (Website/admin/Portal/SiteSettings.ascx.vb); Email format checked
// only when supplied (Library/Components/Portal/PortalInfo.vb carries no validation attributes — rules ported as
// FluentValidation per the migration's Application layer). Default FluentValidation messages are asserted (no
// fabricated text); invalid-email fixtures contain no '@' for deterministic failure.
public class UpdatePortalValidatorTests
{
    private readonly UpdatePortalValidator _validator = new();

    /// <summary>
    /// Builds a fully valid <see cref="UpdatePortalDto"/> baseline that satisfies every rule, so each test can
    /// mutate exactly one property to isolate the rule under examination.
    /// </summary>
