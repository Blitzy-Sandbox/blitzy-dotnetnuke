...
// MIGRATION: Parity tests for UpdateRoleValidator. Rules and messages are identical to
// CreateRoleValidator (UpdateRoleRequest simply omits PortalId), migrated from legacy DNN
// role-edit validation (Website/admin/Security/EditRoles.ascx.vb) and RoleInfo.vb. PARITY QUIRK:
// billing/trial PERIOD rules are guarded by .When(period != 0), so a period of 0 PASSES.
// Tests encode parity ONLY.
public sealed class UpdateRoleValidatorTests
{
    ...
    private static UpdateRoleRequest Valid() => new()
    {
