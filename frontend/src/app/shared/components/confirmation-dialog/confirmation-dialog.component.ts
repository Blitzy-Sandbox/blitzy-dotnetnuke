 ...
 *
 * MIGRATION: the legacy `ClientAPI.AddButtonConfirm(cmdDelete, confirmString)` wired a configurable
 * yes/no browser `confirm()` ("DeleteItem" default, "ConfirmUnRegister" for user-unregister) onto every
 * destructive admin-grid action. That blocking, inaccessible dialog is reproduced here as an accessible,
 * reusable in-app modal whose configurable `message`/`confirmText` inputs preserve the same semantics.
 */
@Component({
