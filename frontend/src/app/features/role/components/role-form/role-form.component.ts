...
export class RoleFormComponent implements OnInit {
  ...
  private patchForm(role: Role): void {
    ...
    // MIGRATION: the wire contract (Role in ../../models/role.model.ts) types serviceFee /
    // billingPeriod / trialFee / trialPeriod as `number | null` because the physical DNN [Roles]
    // columns are NULL-able. The reactive controls are non-nullable (NonNullableFormBuilder), so a
    // null wire value is coalesced to the same default the control was constructed with (fee -> 0,
    // period -> 1) — preserving the legacy "unset => default" semantics.
  ...
  private buildCreateRole(): CreateRole {
    ...
    // sources the current portal from the authenticated user.
    // MIGRATION: the User wire contract (core/models/user.model.ts) exposes the portal id as
    // `portalID` — System.Text.Json's camelCase policy lowercases only the first character of the
    // C# `PortalID` property and leaves the trailing acronym intact — so the field is read as
    // `portalID` (not `portalId`).
  ...
  private buildUpdateRole(id: number, loaded: Role): UpdateRole {
    ...
    // (RoleService.updateRole contract requires dto.roleID === id).
    return {
      ...loaded,
      roleID: id,
