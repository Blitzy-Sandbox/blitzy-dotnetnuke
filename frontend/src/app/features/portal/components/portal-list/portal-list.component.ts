...
export class PortalListComponent implements OnInit {
   ...
   * MIGRATION: the User wire model exposes the owning portal as `portalID`
   * (System.Text.Json camelCase of C# UserDto.PortalID, per core/models/user.model.ts),
   * so the current portal is read from `currentUser()?.portalID`.
   */
